#include "ros/ros.h"
#include <math.h>
#include "nav_msgs/Path.h"										// to get desired path
#include "nav_msgs/Odometry.h"												// to read current position and velocity
...
{
	ros::init(argc, argv, "mower_control");		// connect to roscore
	ros::NodeHandle n;													// node object
	
	// register sub to get desired position/pose commands
	ros::Subscriber ComPose_sub = n.subscribe("/path", 5, ComPoseCallback);
	// register sub to get current position/pose
	ros::Subscriber CurPose_sub = n.subscribe("/odom", 5, CurPoseCallback);
	// register pub to send twist velocity (cmd_vel)
	ros::Publisher Twist_pub = n.advertise<mower::Velocity>("/cmd_vel", 100);
	
	ros::Rate loop_rate(10);											// freq to run loops in (10 Hz)
	float ErrorLin = 0;													// holds linear error
	float ErrorAng = 0;													// holds angular error
	turtlesim::odom cmd_vel;											// holds command velocity to publish
